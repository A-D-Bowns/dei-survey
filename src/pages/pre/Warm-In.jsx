...
export default function WarmIn() {
  const nav = useNavigate();
  const { answers, setAnswer } = useSurvey();

  // We store as a number 0–10. If not set yet, keep it undefined so the user must touch the slider once.
  const value = answers.warm_in;
  const canContinue = typeof value === "number" && value >= 0 && value <= 10;

  const handleChange = (e) => {
    const num = Number(e.target.value);
    if (!Number.isNaN(num)) setAnswer("warm_in", num);
  };
      ...
      <div className="mx-auto max-w-3xl px-4 pb-16 pt-10 md:px-6">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
          ...
          <p className="mt-3 text-gray-700">
            ...
            person who <span className="font-medium">shares your position</span> on workplace DEI?
          </p>
          ...
          <div className="mt-8">
            ...
            <input
              ...
              aria-label="Warmth toward in-group (0 to 10)"
            />
          ...
          <div className="mt-8 flex items-center justify-between">
            <button
              onClick={() => nav("/pre/complexity")} // back to P6
