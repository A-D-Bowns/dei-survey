...
export default function StrengthSupport() {
  ...
  const value = answers.strength_support ?? "";

  const onNext = () => { if (value) nav("/pre/info-oppose"); };

  return (
    <div className="min-h-screen bg-neutral-100/70">
      <div className="mx-auto max-w-3xl px-4 pb-16 pt-10 md:px-6">
        <header className="mb-8">
          <h1 className="text-3xl font-semibold tracking-tight text-gray-900">DEI Study — Pre Survey</h1>
          <p className="text-sm text-gray-500">Single-question pages</p>
        </header>

        {/* Progress: Welcome(0), Consent(1), Stance(2), InfoSupport(3), Strength(4), Complexity(5) */}
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
          ...
          <RadioList
            name="strength_support"
            options={OPTIONS}
            value={value}
            onChange={(v) => setAnswer("strength_support", v)}
          />
